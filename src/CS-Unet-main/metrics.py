...
def dice3D(img_gt, img_pred, voxel_size):
    """
    Function to compute the metrics between two segmentation maps given as inputs.
    ...
    res = []
    # Loop on each classes of the inputs images
    for c in [3, 1, 2]:
        # Copy the gt image to not alterate the inputs
        gt_c_i = np.copy(img_gt)
        gt_c_i[gt_c_i != c] = 0

        # Copy the pred image to not alterate the inputs
