...
class WindowAttention(nn.Module):
    ...
    Args:
        dim (int): Number of inputs channels.
    ...
    def forward(self, x, mask=None):
        """
        Args:
            x: inputs features with shape of (num_windows*B, N, C)
...
class ConvSwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if min(self.input_resolution) <= self.window_size:
            # if window size is larger than inputs resolution, we don't partition windows
            self.shift_size = 0
    ...
    def forward(self, x):
        H, W = self.input_resolution
        B, L, C = x.shape
        assert L == H * W, "inputs feature has wrong size"
...
class PatchMerging(nn.Module):
    ...
    Args:
        input_resolution (tuple[int]): Resolution of inputs feature.
        dim (int): Number of inputs channels.
        norm_layer (nn.Module, optional): Normalization layer.  Default: nn.LayerNorm
    """
...
class PatchEmbed(nn.Module):
    ...
    Args:
        ...
        in_chans (int): Number of inputs image channels. Default: 3.
...
class ConvSwinTransformerSys(nn.Module):
    ...
    def up_x4(self, x):
        ...
        B, L, C = x.shape
        assert L == H * W, "inputs features has wrong size"
